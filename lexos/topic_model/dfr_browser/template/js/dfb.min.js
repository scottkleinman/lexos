...
var view = (function () {
  ...
  u = function (e, t) {
    ...
    if (VIS.svg.has(e)) {
      i = VIS.svg.get(e);
      d3.select(e + " svg")
        .attr("width", t.w + t.m.left + t.m.right)
        .attr("height", t.h + t.m.top + t.m.bottom);
  ...
  f = function (e, t) {
    return e
      ...
      .attr("height", t.h + t.m.top + t.m.bottom)
      .append("g")
      .attr("transform", "translate(" + t.m.left + "," + t.m.top + ")");
  };
  e.append_svg = f;
  p = function () {
...
metadata.key = {
  ...
  continuous: function (e, t) {
    ...
    if (o >= 1) {
      if (n <= 0) {
        r = d3.format(
          "0" + String(o + Math.abs(n) + 1) + "." + String(Math.abs(n)) + "f"
        );
      } else {
...
var bib = function (e) {
  var t = e || {},
    ...
    a;
  if (!t.sorting) {
    t.sorting = [["all_raw", "by raw entry"]];
  }
  n = function (e) {
    if (e !== undefined) {
      t.sorting = e;
  ...
  o = function (e) {
    var t = [],
      n,
      o = e.dir.major ? d3.ascending : d3.descending,
    ...
    n = e.docs
      ...
      .sort(function (e, t) {
        return (
          o(e.major, t.major) || r(e.minor, t.minor) || d3.ascending(e.id, t.id)
        );
      });
...
view.bib = function (e) {
  ...
  d3.select("a#bib_sort_dir").attr(
    "href",
    "#/bib/" + e.major + "/" + e.minor + "/" + (e.dir === "up" ? "down" : "up")
  );
  ...
  i = d3
    .select("#bib_view #bib_headings ul")
    .selectAll("li")
    .data(t, function (e) {
      return e.key;
    });
  ...
  i.selectAll("a")
    ...
    .on("click", function (e) {
      d3.event.preventDefault();
      d3.select("#" + view.bib.id(e.heading))
        .node()
        .scrollIntoView();
    })
    .text(function (e) {
...
view.doc = function (e) {
  ...
  // var doc_url = document.URL.split("modules")[0] + "project_data"; t.select("#doc_remark a.url").attr("href", doc_url + "/" + e.url);
  var doc_url = document.URL.split("modules")[0];
  t.select("#doc_remark a.url").attr("href", doc_url + "/" + e.url);
...
view.model.conditional = function (e) {
  ...
  d3.select(
    t ? "#nav_model_conditional_raw" : "#nav_model_conditional_frac"
  ).classed("active", true);
};
view.model.conditional_plot = function (e) {
  ...
  if (u.size() === 0) {
    u = d3
      .select("#model_view_conditional svg")
      .append("clipPath")
  ...
  d3.select("#model_conditional_clip rect")
    .transition()
    .duration(2e3)
    .attr("width", t.w)
    .attr("height", t.h);
  c = (function () {
    ...
    return function (e, n) {
      var o = t(i[e] % 20);
      return n ? d3.hsl(o).brighter(0.5).toString() : o;
    };
  })();
  if (e.condition_type === "ordinal") {
    n = d3.scale
      .ordinal()
      .domain(e.domain_x)
      .rangeRoundBands([0, t.w], t.ordinal.bar);
    f = "g";
  } else {
  ...
  d.enter()
    ...
    .classed("axis", true)
    .classed("x", true)
    .attr("transform", "translate(0," + t.h + ")");
  d.transition()
    .duration(2e3)
    .attr("transform", "translate(0," + t.h + ")")
    .call(r);
  if (e.condition_type !== "time") {
    ...
    a.enter().append("text");
    a.classed("axis_label", true)
      .attr("x", t.w / 2)
      .attr("y", t.h + t.m.bottom)
      .attr("text-anchor", "middle")
      .text(e.condition_name);
  }
...
view.model.list = function (e) {
  ...
  if (!VIS.ready.model_list) {
    ...
    d3.select("th#model_view_list_condition a").text(
      e.type === "time" ? "over time" : "by " + e.condition_name
    );
  ...
  s =
    e.dir ||
    (d === VIS.last.model_list.sort ? VIS.last.model_list.dir : "up") ||
    "up";
  if (d === "words") {
  ...
  if (e.type === "scaled") {
    ...
    view.model
      .grid_coords({
        n: h,
        cols:
          VIS.model_view.plot.cols ||
          Math.floor(Math.sqrt(VIS.model_view.plot.aspect * h)),
...
view.settings = function (e) {
  ...
  t.select("#n_words_list input")
    .property("min", 1)
    .property("max", e.max_words)
    .property("value", VIS.overview_words)
    .on("change", function () {
      VIS.overview_words = this.valueAsNumber;
    });
  t.select("#n_words_topic input")
    .property("min", 1)
    .property("max", e.max_words)
  ...
  t.select("#conditional_streamgraph")
...
view.topic.remark = function (e) {
  d3.select("#topic_view #topic_remark").text(
    VIS.percent_format(e.col_sum / e.total_tokens) + " of corpus"
  );
};
view.topic.words = function (e) {
  ...
  t.append("td")
    ...
    .text(function (e) {
      let length = 14;
      return e.word.length > length
        ? e.word.substring(0, length - 3) + "..."
        : e.word;
    });
  //
  view.append_weight_tds(t, function (t) {
...
view.topic.conditional_barplot = function (e) {
  var t,
    ...
    m,
    w,
    _ = e.dirty ? e.spec.tx_duration : 0,
    v = e.svg,
    h = e.spec;
  t = e.data
    .keys()
  ...
  if (e.type === "continuous") {
    ...
    d = d3.scale
      .linear()
      .domain([t[0].x, t[t.length - 1].x + i * h.continuous.bar.w])
      .range([0, h.w]);
    ...
    d = d3.time.scale
      ...
      .domain([
        t[0].x,
        d3.time[h.time.bar.unit].utc.offset(t[t.length - 1].x, h.time.bar.w),
      ])
      .range([0, h.w]);
    n =
      d(d3.time[h.time.bar.unit].utc.offset(t[0].x, h.time.bar.w)) - d(t[0].x);
    r = -n / 2;
    o =
      d(d3.time[h.time.step.unit].utc.offset(t[0].x, h.time.step.n)) -
      d(t[0].x);
    a = -o / 2;
    m = o / 2;
  } else {
  ...
  if (e.axes) {
    ...
    p.enter().append("text");
    p.classed("axis_label", true)
      .attr("x", h.w / 2)
      .attr("y", h.h + h.m.bottom)
      .attr("text-anchor", "middle")
      .text(e.condition_name);
  }
    ...
    u.on("mouseover", function (e) {
      ...
      .on("click", function (t) {
        if (d3.select(this.parentNode).classed("selected_condition")) {
          ...
          d3.selectAll(".selected_condition").classed(
            "selected_condition",
            false
          );
...
view.topic.dropdown = function (e) {
  ...
  t.sort(function (e, t) {
    return d3.ascending(
      view.topic.sort_name(e.label),
      view.topic.sort_name(t.label)
    );
  });
...
view.word = function (e) {
  ...
  u = d3.scale
    .linear()
    .domain([0, Math.max(1, e.topics.length - 1)])
    .range([a, a * (Math.max(e.topics.length, 1) + 1)]);
  f = d3.scale
    .linear()
    .domain([0, 1])
    .range([0, a / 2]);
  ...
  c.select("rect")
    .attr("x", -r.m.left)
    .attr("y", -r.m.top)
    .attr("width", r.w + r.m.left + r.m.right)
    .attr("height", r.h + r.m.top + r.m.bottom);
  ...
  p.exit()
    .transition()
    .duration(2e3)
    .attr("transform", "translate(0," + a * (n + p.exit().size()) + ")")
    .remove();
  m.append("rect")
    ...
    function (t, i) {
      ...
      return n.map(function (e, t) {
        return {
          word: e,
          y:
            -a / 2 - (n.length / 2 - t - 1) * VIS.word_view.topic_label_leading,
        };
      });
    },
    function (e, t) {
  ...
  w.exit().remove();
  w.attr("x", -VIS.word_view.topic_label_padding)
    .attr("y", function (e) {
      return e.y;
    })
    .text(function (e) {
...
var dfb = function (e) {
  ...
  t.views.set("word", function (e) {
    ...
    if (n) {
      ...
      if (VIS.last.word) {
        n = VIS.last.word;
        i.select("a#last_word")
          .attr("href", "#/word/" + n)
          .text(document.URL.replace(/#.*$/, "") + "#/word/" + n);
  ...
  t.views.set("doc", function (e) {
    ...
    if (!isFinite(n) || n < 0 || n >= t.m.n_docs()) {
      ...
      i.select("a#last_doc")
        .attr("href", "#/doc/" + n)
        .text(document.URL.replace(/#.*$/, "") + "#/doc/" + n);
